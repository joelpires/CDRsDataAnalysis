...
-- issue: ver o impacto que tem o facto de que o que conta no call_dim é o cell_unique_id e nao o cell_id
...
-- issue: pôr DROP IF DOES NOT EXIST  em praticamente todas as tabelas
-- issue: incluir utilizadores da madeira e dos açores
-- issue: distinguish Braga municipal from Guimarães
-- issue: porque na regularidade eu tenho de tirar os ultimos 42 valores
-- issue:  sumdiffDias podia ser melhor calculado, bastanto a difereneça entre a primeira e a ultima chamada registada pelo utilizador
-- issue: na parte das experiencias fazer so uma função para as 3 variaveis parecidas
-- issue: nas experiments4 usar o statsmunicipals em vez de infomunicipals (sem perda de informacao claro)
...
SELECT * FROM infoMunicipals ORDER BY averageKm2PerCell ASC;
----------------------------------------------------------
...
CREATE TEMPORARY TABLE experiment11 AS (
   SELECT *
    FROM call_fct_porto_weekdays p
    INNER JOIN (SELECT * FROM travelTimes_H_W) h ON hwid = p.id
...
DELETE
FROM experiment11 u
USING home_workplace_by_user d
WHERE (cell_id = home_id AND cell_id = workplace_id);
...
CREATE TEMPORARY TABLE experiment1 AS (
   SELECT *
    FROM call_fct_porto_weekdays p
    INNER JOIN (SELECT * FROM travelTimes_W_H) h ON whid = p.id
...
DROP TABLE ODporto_users_characterization;
...
CREATE TABLE porto_possible_routes (
  userID                 INTEGER,
  commutingType          TEXT,
  routeNumber            INTEGER,
  duration               INTEGER,
...
CREATE TABLE lisbon_possible_routes (
  userID                 INTEGER,
  commutingType          TEXT,
  routeNumber            INTEGER,
  duration               INTEGER,
...
CREATE TABLE coimbra_possible_routes (
  userID                 INTEGER,
  commutingType          TEXT,
  routeNumber            INTEGER,
  duration               INTEGER,
...
CREATE TEMPORARY TABLE frequencies_intermediateTowers_H_W AS (
...
CREATE TEMPORARY TABLE frequencies_intermediateTowers_W_H AS (
  SELECT id, cell_id, latitude, longitude, count(date_id) AS frequencia
  FROM intermediateTowers_W_H
...
CREATE TEMPORARY TABLE durationsScores AS (
  SELECT *,
          CASE
                ...
                ELSE 2*abs(traveltime-duration)
